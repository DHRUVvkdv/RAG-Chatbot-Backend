...
def get_google_drive_link_pdf(pdf_name):
    try:
        response = requests.get(f"{FILE_ID_SERVICE_URL}/file_id", params={
            "file_type": "pdf",
            "file_name": pdf_name
        }, timeout=5)
        ...
        logging.error(f"Unexpected error fetching Google Drive link for {pdf_name}: {str(e)}")
        return ""
    ...
    while True:
        try:
            ...
            for match in query_response.matches:
                ...
                if 'google_drive_link' not in new_metadata or not new_metadata['google_drive_link']:
                    should_update = True
            ...
            logging.info(f"Updated {len(updates)} vectors in this batch. Total updated: {total_updated}")
